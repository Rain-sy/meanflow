"""
Dual-Pipeline SR Evaluation Script
...
# ============================================================================
# Model Components (same as train_sr_dual.py)
# ============================================================================
...
# ============================================================================
# Main Pipeline (DiT with Input Conditioning)
# ============================================================================
...
class MainPipeline(nn.Module):
    def __init__(
        ...
        # 6 channels input (z_t + lr_cond)
        self.patch_embed = PatchEmbed(patch_size, in_channels * 2, hidden_size)
    ...
    def forward(self, z_t, lr_cond, t, h):
        x_input = torch.cat([z_t, lr_cond], dim=1)
...
class DualPipelineSR(nn.Module):
    ...
    def forward(self, z_t, lr_cond, t, h):
        velocity = self.main_pipeline(z_t, lr_cond, t, h)
        return velocity
...
def analyze_training(checkpoint_path):
    """Analyze training convergence from checkpoint"""
    ...
    print("=" * 50)
    print(f"Epochs trained: {info['epoch'] + 1 if isinstance(info['epoch'], int) else info['epoch']}")
    print(f"Final loss: {info['loss']:.6f}" if isinstance(info['loss'], float) else f"Final loss: {info['loss']}")
    print(f"Best PSNR: {info['psnr']:.2f} dB" if isinstance(info['psnr'], float) else f"Best PSNR: {info['psnr']}")
    print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(description='Dual-Pipeline SR Evaluation')
    ...
    parser.add_argument('--analyze', action='store_true', help='Analyze training convergence')
    
    args = parser.parse_args()
    device = torch.device(args.device if torch.cuda.is_available() else 'cpu')
    
    # Analyze training if requested
    if args.analyze:
        analyze_training(args.checkpoint)
    ...
    # Output dir
    output_dir = os.path.join(args.output_base, args.dataset, 'DualPipeline', exp_name)
    ...
    print("=" * 70)
    print("Dual-Pipeline SR Evaluation")
    print("=" * 70)
    ...
    # Build model
    model = MODEL_CONFIGS[args.model_size](img_size=args.tile_size, use_flux=False)
    ...
    for hf, lf in tqdm(zip(hr_files, lr_files), total=len(hr_files), desc="Evaluating"):
        ...
        if args.save_comparisons:
            # Get original LR image size
            lr_W, lr_H = lr_img.size
            # Comparison: LR (original size) | Bicubic | Ours | HR
            comp = Image.new('RGB', (W * 4, H))
            # LR: resize to HR size for display
    ...
    if avg_psnr > avg_psnr_bic + 0.5:
        print("✓ Model shows improvement over bicubic (+{:.2f} dB)".format(avg_psnr - avg_psnr_bic))
    else:
        print("⚠ Model shows minimal improvement. May need more training.")
    
    if 'loss' in ckpt and ckpt['loss'] < 0.01:
        print("✓ Loss is low ({:.6f}), model likely converged".format(ckpt['loss']))
    elif 'loss' in ckpt:
        print("⚠ Loss is {:.6f}, may need more training".format(ckpt['loss']))
    ...
    with open(results_path, 'w') as f:
        f.write("Dual-Pipeline SR Evaluation Results\n")
        f.write("=" * 50 + "\n")
