"""
PMF-MeanFlow-MMDiT Evaluation

Compatible with the dual-head (u + v) model from train_sr_mmdit_pmf.py.

Output structure:
    outputs/
    └── DIV2K/
        └── PMF_MMDiT_small_1step_x4/
            ├── predictions/
            ├── comparisons/
            └── results.txt

Usage:
    python evaluate_sr_pmf.py \
        --checkpoint checkpoints_pmf_mmdit/xxx/best_model.pt \
        --hr_dir Data/DIV2K/DIV2K_valid_HR \
        --lr_dir Data/DIV2K/DIV2K_valid_LR_bicubic_X4 \
        --model_size small \
        --scale 4 \
        --num_steps 1   --device cuda:0 \
"""
...
# ============================================================================
# Model (same as training - PMF dual-head version)
# ============================================================================
...
class PMFMeanFlowMMDiT(nn.Module):
    """PMF-improved MMDiT with dual output heads (u + v)."""
    ...
    def __init__(self, img_size=128, patch_size=4, in_channels=3,
        ...
        # Dual heads
        self.u_head = FinalLayer(hidden_size, patch_size, in_channels)
        self.v_head = FinalLayer(hidden_size, patch_size, in_channels)
...
def PMFMeanFlowMMDiT_XS(img_size=128): return PMFMeanFlowMMDiT(img_size, hidden_size=256, depth=8, num_heads=4)
def PMFMeanFlowMMDiT_S(img_size=128): return PMFMeanFlowMMDiT(img_size, hidden_size=384, depth=12, num_heads=6)
def PMFMeanFlowMMDiT_B(img_size=128): return PMFMeanFlowMMDiT(img_size, hidden_size=512, depth=12, num_heads=8)
def PMFMeanFlowMMDiT_L(img_size=128): return PMFMeanFlowMMDiT(img_size, hidden_size=768, depth=24, num_heads=12)

MODEL_CONFIGS = {'xs': PMFMeanFlowMMDiT_XS, 'small': PMFMeanFlowMMDiT_S, 'base': PMFMeanFlowMMDiT_B, 'large': PMFMeanFlowMMDiT_L}
...
def run_sr_single(model, lr_tensor, device, num_steps=1):
    """Run SR on single image (with padding). Uses u-head only."""
    ...
    for i in range(num_steps):
        ...
        u, _ = model(x, lr_padded, t, h)  # only use u-head
        x = x - dt * u
...
def run_sr_tiled(model, lr_tensor, device, num_steps=1, tile_size=128, overlap=16):
    """Run SR with tiling for large images. Uses u-head only."""
    ...
    for h_start in h_starts:
        for w_start in w_starts:
            ...
            for i in range(num_steps):
                ...
                u, _ = model(x, tile_padded, t, h_param)  # u-head only
                x = x - dt * u
...
def main():
    parser = argparse.ArgumentParser(description='PMF-MeanFlow-MMDiT Evaluation')
    ...
    model_name = f'PMF_MMDiT_{args.model_size}'
    ...
    print("=" * 70)
    print("PMF-MeanFlow-MMDiT Evaluation")
    print("=" * 70)
    ...
    if 'psnr_1step' in ckpt:
        print(f"Training PSNR: 1-step={ckpt['psnr_1step']:.2f}dB, 10-step={ckpt.get('psnr_10step', 0):.2f}dB")
    ...
    for hf, lf in tqdm(zip(hr_files, lr_files), total=len(hr_files)):
        ...
        results_per_image.append({
            'name': hf, 'psnr': psnr_val, 'ssim': ssim_val,
            'psnr_bicubic': psnr_bic, 'ssim_bicubic': ssim_bic,
    ...
    print("-" * 70)
    print(f"{'Bicubic':<25} {avg_psnr_bic:<15.4f} {avg_ssim_bic:<15.4f}")
    print(f"{model_name:<25} {avg_psnr:<15.4f} {avg_ssim:<15.4f}")
    print("-" * 70)
    print(f"{'Improvement':<25} {avg_psnr - avg_psnr_bic:+.4f} dB        {avg_ssim - avg_ssim_bic:+.4f}")
    print("=" * 70)
    
    results_file = os.path.join(output_dir, 'results.txt')
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write("PMF-MeanFlow-MMDiT Evaluation Results\n")
        f.write("=" * 70 + "\n")
        f.write(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Dataset: {args.dataset}\n")
        f.write(f"Model: {model_name}\n")
        f.write(f"Checkpoint: {args.checkpoint}\n")
        f.write(f"Scale: {args.scale}x, Steps: {args.num_steps}\n")
